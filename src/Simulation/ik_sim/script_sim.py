...
from Simulation.ik_sim.ik_pyroki import solve_ik_pyroki, set_pyroki

def conect_to_sim(sim_type, gui=False):
    if sim_type == 'xarm':
        import Simulation.ik_sim.ufactory_sim as m
        arm = m.set_xarm()
    elif sim_type == 'pybullet':
        import Simulation.ik_sim.pybullet_sim as m
        arm = m.set_pybullet(gui=gui)
    else:
...
def calc_ik(arm_sim_type, ik_solver, use_curr_joints, n_random_starts, expected_poses, joint_angles):
    ...
    for i, (rec_pose, rec_joint_angle) in enumerate(zip(expected_poses, joint_angles)):
        ...
        if ik_solver == 'pyroki':
            ik_joints_deg, shift_chosen, calc_done = solve_ik_pyroki(solve_fn, ik_robot, ik_target_link_index, curr_joints_red, rec_pose, reg_weights=(0.0005), n_random_starts=n_random_starts)
...
if __name__ == "__main__":
    ...
    # the_title = f'Self-colition=True-Box, init joints={n_random_starts}'
    # the_title = f'Self-colition=False, init joints={n_random_starts}'
    the_title = f'Self-colition=Ture-STL, init joints={n_random_starts}'
